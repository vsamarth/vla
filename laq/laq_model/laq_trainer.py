...
class LAQTrainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.valid_dl = DataLoader(self.valid_ds, batch_size=batch_size, num_workers=4)
        ...
        if exists(self.vae.discr):
            ...
            (self.vae, self.optim, self.dl) = self.accelerator.prepare(
                self.vae, self.optim, self.dl
            )
        ...
        if self.is_main and not (steps % self.save_results_every):
            ...
            for model, filename in vaes_to_evaluate:
                ...
                if self.train_on_images:
                    ...
                    imgs_and_recons = torch.stack(
                        (valid_data[:, :, 0], valid_data[:, :, -1], recons), dim=0
                    )
                    # imgs_and_recons = torch.stack((valid_data, recons), dim = 0)
                    imgs_and_recons = rearrange(imgs_and_recons, "r b ... -> (b r) ...")
                    ...
                    grid = make_grid(
                        imgs_and_recons, nrow=3, normalize=True, value_range=(0, 1)
                    )
